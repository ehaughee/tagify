...
class Playlists {
   ...
   */
  async getPlaylist(playlistId: string, cache = true): Promise<SpotifyApi.PlaylistObjectFull> {
    return new Promise((resolve, reject) => {
      ...
      console.log(`[Cache] ${cache ? 'MISS' : 'SKIP'} for playlist '${playlistId}'`);
   ...
   */
  async getPlaylistTracks(playlistId: string, offset = 0, limit = 100, cache = true): Promise<SpotifyApi.PlaylistTrackObject[]> {
    return new Promise((resolve, reject) => {
      if (cache && this.cacheSpanInitialized(this.playlistCache?.[playlistId]?.tracks.items, offset, limit)) {
        console.log(`[Cache] HIT for playlist '${playlistId}[${offset}, ${offset + limit}]'`);

        resolve(this.playlistCache[playlistId]?.tracks.items.slice(offset, offset + limit));
        return;
      }

      console.log(`[Cache] ${cache ? 'MISS' : 'SKIP'} for playlist '${playlistId}[${offset}, ${offset + limit}]'`);

      this.spotifyApi.getPlaylistTracks(playlistId, { offset: offset, limit: limit})
      .then(response => {
        this.playlistCache[playlistId]?.tracks.items.splice(offset, limit, ...response.items);
        resolve(response.items);
      })
      .catch(e => reject(e));
    });
